...
describe("activateWorldInfo recursion settings", () => {
  test("records activation trace before final priority sorting", () => {
    const firstTriggered = makeEntry({
      id: "first-triggered",
      uid: "first-triggered",
      key: ["alpha"],
      priority: 1,
    ...
    const result = activateWorldInfo({
      entries: [firstTriggered, sortedFirst],
      messages: [makeMessage("alpha")],
      chatTurn: 1,
      wiState: {},
    });

    expect(result.activationTrace).toEqual(["first-triggered", "sorted-first"]);
    expect(result.activatedEntries.map((entry) => entry.id)).toEqual(["sorted-first", "first-triggered"]);
  });
...
describe("mergeActivatedWorldInfoEntries — user-shape (9000+ entries, keys=[], vectorized)", () => {
  test("annotates activation order and first-triggered entry per book", () => {
    const first = makeEntry({
      id: "book-a-first",
      uid: "book-a-first",
      world_book_id: "book-a",
    ...
    const higherPriority = makeEntry({
      id: "book-a-higher",
      uid: "book-a-higher",
      world_book_id: "book-a",
      key: ["alpha"],
    ...
    const otherBook = makeEntry({
      id: "book-b-first",
      uid: "book-b-first",
      world_book_id: "book-b",
      key: ["alpha"],
      vectorized: false,
    });

    const result = mergeActivatedWorldInfoEntries(
      [higherPriority, first, otherBook],
      [],
      {},
      undefined,
      new Map([["book-a", "Book A"], ["book-b", "Book B"]]),
      [first.id, otherBook.id, higherPriority.id],
    );

    const byId = new Map(result.activatedWorldInfo.map((entry) => [entry.id, entry]));
    expect(byId.get(first.id)?.activationOrder).toBe(0);
    expect(byId.get(first.id)?.firstTriggeredForBook).toBe(true);
    expect(byId.get(higherPriority.id)?.activationOrder).toBe(2);
    expect(byId.get(higherPriority.id)?.firstTriggeredForBook).toBe(false);
    expect(byId.get(otherBook.id)?.firstTriggeredForBook).toBe(true);
  });
