...
export class WorldBookEntryConflictError extends Error {
  ...
  constructor(payload: WorldBookEntryConflictPayload) {
    super("World book entry revision conflict");
...
function assertExpectedRevisions(
  ...
  const conflicts = requestedIds.flatMap((id) => {
    const current = entryMap.get(id) ?? null;
    const expected = expectedRevisions?.[id];
    if (current && Number.isInteger(expected) && expected === current.revision) return [];
    return [{ id, current }];
  });
  if (conflicts.length > 0) {
    throw new WorldBookEntryConflictError({
...
export function updateEntry(userId: string, id: string, input: UpdateWorldBookEntryInput): WorldBookEntry | null {
  ...
  const result = getDb().query(`UPDATE world_book_entries SET ${fields.join(", ")} WHERE ${where}`).run(...values);
...
export function reorderEntries(
  ...
  assertExpectedRevisions(entries, expectedRevisions, uniqueIds);
...
export function bulkOperateEntries(
  ...
  assertExpectedRevisions(entries, input.expected_revisions, uniqueIds);

  const entryById = new Map(entries.map((entry) => [entry.id, entry]));
  const orderedEntries = uniqueIds.map((id) => entryById.get(id)!);
  const now = Math.floor(Date.now() / 1000);
  const db = getDb();
  ...
  if (input.action === "move") {
    ...
    db.transaction(() => {
      const stmt = db.query(
        `UPDATE world_book_entries
         SET world_book_id = ?, updated_at = ?, vector_index_status = ?, vector_indexed_at = NULL,
             vector_index_error = NULL, revision = revision + 1
  ...
  if (input.action === "copy") {
    ...
    db.transaction(() => {
      for (const entry of orderedEntries) {
        const copy = createEntry(userId, targetBook.id, {
        ...
        if (!copy) throw new Error("Target world book not found");
        created.push(copy);
      }
      touchWorldBook(targetBook.id, now);
    })();
    for (const entry of created) emitWorldBookEntryChanged(userId, entry.id);
    emitWorldBookChanged(userId, targetBook.id);
    return { action: input.action, affected: created.length, target_book_id: targetBook.id };
  }
  ...
  if (input.action === "set_position") {
    ...
    db.transaction(() => {
      const stmt = db.query(
        "UPDATE world_book_entries SET position = ?, depth = ?, updated_at = ?, revision = revision + 1 WHERE id = ? AND world_book_id = ?",
      );
      orderedEntries.forEach((entry) => {
        stmt.run(position, position === 4 ? depth : entry.depth, now, entry.id, worldBookId);
      });
      touchWorldBook(worldBookId, now);
    })();
    emitWorldBookChanged(userId, worldBookId);
  ...
  if (input.action === "set_priority") {
    ...
    db.transaction(() => {
      const stmt = db.query(
        "UPDATE world_book_entries SET priority = ?, updated_at = ?, revision = revision + 1 WHERE id = ? AND world_book_id = ?",
      );
      uniqueIds.forEach((entryId) => stmt.run(priority, now, entryId, worldBookId));
      touchWorldBook(worldBookId, now);
    })();
    emitWorldBookChanged(userId, worldBookId);
    return { action: input.action, affected: uniqueIds.length };
  ...
  if (input.action === "set_depth") {
    const depth = Number.isFinite(input.depth) ? Math.max(0, Math.trunc(input.depth)) : 0;
    db.transaction(() => {
      const stmt = db.query(
        "UPDATE world_book_entries SET depth = ?, updated_at = ?, revision = revision + 1 WHERE id = ? AND world_book_id = ?",
      );
      uniqueIds.forEach((entryId) => stmt.run(depth, now, entryId, worldBookId));
      touchWorldBook(worldBookId, now);
    })();
    emitWorldBookChanged(userId, worldBookId);
    return { action: input.action, affected: uniqueIds.length };
  ...
  if (input.action === "set_trigger") {
    const constant = input.trigger === "constant";
    const vectorized = input.trigger === "vector";
    db.transaction(() => {
      const stmt = db.query(
        `UPDATE world_book_entries
         SET constant = ?, vectorized = ?, vector_index_status = ?,
      ...
      orderedEntries.forEach((entry) => {
        stmt.run(
          constant ? 1 : 0,
          vectorized ? 1 : 0,
          vectorized ? "pending" : "not_enabled",
          now,
          entry.id,
          worldBookId,
        );
    ...
    const vectorizedEntries = input.trigger === "vector" ? orderedEntries : [];
    queueReindexForEntries(userId, vectorizedEntries);
    if (input.trigger !== "vector") {
      for (const entry of orderedEntries) {
        void embeddingsSvc.deleteWorldBookEntryEmbeddings(userId, entry.id).catch(() => {});
      }
    }
    emitWorldBookChanged(userId, worldBookId);
    return { action: input.action, affected: uniqueIds.length };
  ...
  if (input.action === "set_enabled") {
    const nextDisabled = !input.enabled;
    db.transaction(() => {
      const stmt = db.query(
        `UPDATE world_book_entries
         SET disabled = ?, vector_index_status = ?, vector_indexed_at = NULL,
             vector_index_error = NULL, updated_at = ?, revision = revision + 1
         WHERE id = ? AND world_book_id = ?`,
      );
      orderedEntries.forEach((entry) => {
        stmt.run(
          nextDisabled ? 1 : 0,
          entry.vectorized && !nextDisabled ? "pending" : "not_enabled",
          now,
          entry.id,
          worldBookId,
        );
    ...
    queueReindexForEntries(userId, orderedEntries.filter((entry) => entry.vectorized && input.enabled));
    emitWorldBookChanged(userId, worldBookId);
    return { action: input.action, affected: uniqueIds.length };
  }
  ...
  if (input.action === "set_fields") {
    ...
    if (!hasMutation) throw new Error("At least one field is required");
    ...
    db.transaction(() => {
      for (const entry of orderedEntries) {
        ...
        if (input.priority !== undefined) {
          ...
          values.push(Math.trunc(input.priority));
        }
        if (input.depth !== undefined) {
          fields.push("depth = ?");
          values.push(Math.max(0, Math.trunc(input.depth)));
        }
        if (input.position !== undefined) {
          const position = Math.trunc(input.position);
          if (position < 0 || position > 7) throw new Error("position must be between 0 and 7");
          fields.push("position = ?");
          values.push(position);
        }
        if (input.trigger !== undefined) {
          const vectorized = input.trigger === "vector";
          fields.push("constant = ?", "vectorized = ?", "vector_index_status = ?", "vector_indexed_at = NULL", "vector_index_error = NULL");
          values.push(input.trigger === "constant" ? 1 : 0, vectorized ? 1 : 0, vectorized ? "pending" : "not_enabled");
        }
        if (input.enabled !== undefined) {
          fields.push("disabled = ?");
          values.push(input.enabled ? 0 : 1);
          if (input.trigger === undefined && entry.vectorized) {
            fields.push("vector_index_status = ?", "vector_indexed_at = NULL", "vector_index_error = NULL");
            values.push(input.enabled ? "pending" : "not_enabled");
          }
        }
        fields.push("updated_at = ?", "revision = revision + 1");
        values.push(now, entry.id, worldBookId);
        db.query(
          `UPDATE world_book_entries SET ${fields.join(", ")} WHERE id = ? AND world_book_id = ?`,
        ).run(...values);
      }
      touchWorldBook(worldBookId, now);
    })();

    const shouldVectorize = input.trigger === "vector"
      || (input.trigger === undefined && input.enabled === true);
    if (shouldVectorize) {
      queueReindexForEntries(userId, orderedEntries.filter((entry) => input.trigger === "vector" || entry.vectorized));
    }
    if (input.trigger === "constant" || input.trigger === "keyword" || input.enabled === false) {
      for (const entry of orderedEntries) {
        void embeddingsSvc.deleteWorldBookEntryEmbeddings(userId, entry.id).catch(() => {});
      }
    }
