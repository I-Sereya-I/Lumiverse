...
export function collectWorldInfoSources(
  ...
  if (worldBookIds.length > 0) {
    ...
    for (const id of worldBookIds) {
      ...
      if (book?.name) bookNameMap.set(id, book.name);
...
export function mergeActivatedWorldInfoEntries(
  ...
  bookSourceMap?: Map<string, BookSource>,
  bookNameMap?: Map<string, string>,
  keywordActivationTrace: string[] = keywordEntries.map((entry) => entry.id),
  ...
  for (const entryId of keywordActivationTrace) {
    if (!activationOrderById.has(entryId)) {
      activationOrderById.set(entryId, activationOrderById.size);
    }
  }
  for (const item of vectorEntries) {
    if (!activationOrderById.has(item.entry.id)) {
      activationOrderById.set(item.entry.id, activationOrderById.size);
    }
  }

  const firstEntryByBook = new Map<string, string>();
  for (const entry of [...finalized.activatedEntries].sort(
    (a, b) =>
      (activationOrderById.get(a.id) ?? Number.MAX_SAFE_INTEGER) -
      (activationOrderById.get(b.id) ?? Number.MAX_SAFE_INTEGER),
  )) {
    if (!firstEntryByBook.has(entry.world_book_id)) {
      firstEntryByBook.set(entry.world_book_id, entry.id);
    }
  }
  ...
  const activatedWorldInfo: ActivatedWorldInfoEntry[] =
    finalized.activatedEntries.map((entry) => {
      ...
      return {
        ...
        activationType: source?.source === "vector" ? "vector" : entry.constant ? "constant" : "keyword",
        activationOrder: activationOrderById.get(entry.id) ?? activationOrderById.size,
        firstTriggeredForBook: firstEntryByBook.get(entry.world_book_id) === entry.id,
        estimatedTokens: Math.ceil((entry.content?.length ?? 0) / 4),
