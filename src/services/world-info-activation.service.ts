...
export interface ActivationResult {
  ...
  /** Entry ids in the exact order they activated, before priority sorting/budgeting. */
...
function getCachedActivationResult(cacheKey: string): ActivationResult | null {
  ...
  return {
    ...cached.result,
    activationTrace: [...cached.result.activationTrace],
...
export function activateWorldInfo(input: ActivationInput): ActivationResult {
  ...
  const activationTrace = activated.map((entry) => entry.id);
