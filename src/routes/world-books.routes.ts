...
app.post("/:id/entries/reorder", async (c) => {
  ...
  if (!body?.expected_revisions || typeof body.expected_revisions !== "object") {
    return c.json({ error: "expected_revisions is required" }, 400);
  }
  try {
    ...
    if (err instanceof svc.WorldBookEntryConflictError) {
...
app.put("/:id/entries/:eid", async (c) => {
  ...
  if (!Number.isInteger(body?.expected_revision)) {
    return c.json({ error: "expected_revision is required" }, 400);
  }
  try {
...
app.delete("/:id/entries/:eid", (c) => {
  ...
  try {
    const result = svc.bulkOperateEntries(userId, bookId, {
      action: "delete",
      entry_ids: [entryId],
      expected_revisions: { [entryId]: expectedRevision },
    });
    if (!result) return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
  } catch (err: any) {
