...
Your backend module runs in an isolated Bun subprocess by default. Legacy
worker mode and the platform-dependent sandbox transport are also available.
The `spindle` global is automatically available - no imports needed.
...
| [Shared RPC Pool](shared-rpc-pool.md) | Free | Cross-extension latest-state and on-demand reads across the runtime isolation boundary |
