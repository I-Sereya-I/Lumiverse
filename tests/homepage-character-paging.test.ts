...
describe('mergeCharacterPage', () => {
  ...
  it('drops incoming rows whose id is already held', () => {
    // `sort: 'discover'` reshuffles per request, so offset paging over it hands back
    // overlapping rows. Duplicate ids would mean duplicate React keys in the grid.
  ...
  it('keeps the first occurrence, so the selection fallback target never moves', () => {
    // The hook falls back to `characters[0]` when nothing is explicitly selected. If a
    // later page could replace the head row, appending a page would silently re-select.
...
describe('hasMorePages', () => {
  ...
  it('stops on a short page even when the total claims there is more', () => {
    // The server can over-report `total` when rows are filtered out after counting; a
    // short page is the authoritative "that was the tail".
...
describe('applyCharacterPage — accumulation', () => {
  ...
  it('advances the offset by rows received, not by rows rendered', () => {
    // Under `discover` the deduped list is shorter than what the server handed over.
    // Paging off the rendered length would re-request the overlap forever.
    let state = createCharacterPageState<Row>(1)
...
describe('applyCharacterPage — stale generation', () => {
  it('drops a response whose generation no longer matches, by identity', () => {
    // The scenario: page 1 for "elf" is in flight, the user types "orc", the list resets,
    // and *then* the elf response resolves. It must not splice elves into the orc list.
    ...
    // Identity is the signal the hook checks — `next === state` means "discard".
  ...
  it('drops a late page-2 response that arrives after a reset', () => {
    ...
    // The user changes the sort: the hook builds a fresh state with a new generation.
...
describe('shouldLoadMore', () => {
  ...
  it('refuses while the first page of a new query is in flight', () => {
    // On reset the previous cards are still painted while `received` is back to 0; an
    // intersection in that window would request offset 0 a second time.
...
describe('characterQueryKey', () => {
  ...
  it('is stable across fresh but equal inputs', () => {
    // `favorites` arrives as a new array on unrelated store writes. Keying off identity
    // would dump the user back to page 1 every time.
  ...
  it('ignores whitespace the request trims away', () => {
    // Typing a trailing space must not reset paging: the server sees the same query.
  ...
  it('does not collide when a value moves between fields', () => {
    // A naive `values.join('-')` key would make these two identical.
...
describe('reset semantics end to end', () => {
  it('a query change discards accumulated pages and starts at offset 0', () => {
    ...
    // The hook rebuilds state from scratch when `characterQueryKey` changes.
