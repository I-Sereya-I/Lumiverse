...
- **Productivity surfaces (optional)** — A Quick Toolbar, compact Connections Picker, Lore Indicator, Portrait Dock, and half-screen lorebook workspace can keep frequent actions close to chat. See [Productivity Workflow](productivity-workflow.md).
...
When you open Lumiverse (or navigate to the home page), you land on the **Landing Page**. It shows recent chats and can include the persisted [Homepage Character Library](productivity-workflow.md#homepage-character-library), with configurable card geometry, metadata, sorting, filtering, and a selected-character preview.
