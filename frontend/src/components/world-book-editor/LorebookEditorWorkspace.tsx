...
export default function LorebookEditorWorkspace({
  ...
  const updateSettings = useCallback((patch: Partial<typeof settings>) => {
    setSetting('lorebookEditorSettings', { ...settings, ...patch })
  }, [setSetting, settings])
  ...
  useEffect(() => {
    ...
    const storedVersion = current.entryMetadataVersion ?? 0
    if (storedVersion >= ENTRY_METADATA_VERSION) return
    setSetting('lorebookEditorSettings', {
      ...current,
      visibleEntryMetadata: backfillEntryMetadata(current.visibleEntryMetadata, storedVersion),
  ...
  const applyBulk = useCallback(async () => {
    ...
    await runBulk({ action: 'set_fields', entry_ids: selectedIds, ...patch })
