  ...
  const handleDeleteEntry = useCallback(
    async (entryId: string) => {
      ...
      try {
        ...
        if (!current) return
        await worldBooksApi.deleteEntry(selectedBookId, entryId, current.revision)
        if (selectedEntryId === entryId) setSelectedEntryId(null)
        await loadEntries(selectedBookId, entryPage, entrySortBy, entrySortDir, debouncedEntrySearch)
      } catch {}
    },
    [entries, selectedBookId, selectedEntryId, entryPage, entrySortBy, entrySortDir, debouncedEntrySearch, loadEntries]
  )
    ...
    (entryId: string, updates: Record<string, any>) => {
      ...
      void worldBooksApi.updateEntry(selectedBookId, entryId, {
        ...
        .then((updated) => {
          ...
          return loadVectorSummary(selectedBookId)
        })
        .catch(() => {})
    },
    [entries, selectedBookId, loadVectorSummary]
  )
