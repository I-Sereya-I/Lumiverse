...
export default function ConnectionForm({ providers, profile, onSave, onCancel, onOAuthCreated }: ConnectionFormProps) {
  ...
  const profileTags = useStore((s) => s.connectionsPickerSettings.profileTags)
  const [tagIds, setTagIds] = useState<string[]>(() => profile ? getConnectionProfileTagIds(profile) : [])
  ...
  useEffect(() => {
    ...
    setTagIds(profile ? getConnectionProfileTagIds(profile) : [])
  }, [profile?.id])
  ...
  const handleSubmit = useCallback(() => {
    if (!name.trim()) return
    const metadata: Record<string, any> = { ...profile?.metadata }
    metadata.tagIds = [...new Set(tagIds)]
  ...
  return (
    <div className={styles.form}>
      ...
      {profileTags.length > 0 && (
        <FormField label="Connection tags" hint="Tags group this profile in the Connections Picker.">
          <div className={styles.connectionTagGrid}>
            {[...profileTags].sort((a, b) => a.order - b.order).map((tag) => (
              <label key={tag.id} className={styles.connectionTagOption}>
                <input
                  type="checkbox"
                  checked={tagIds.includes(tag.id)}
                  onChange={(event) => setTagIds((current) => event.target.checked
                    ? [...current, tag.id]
                    : current.filter((id) => id !== tag.id))}
                />
                <span className={styles.connectionTagDot} style={{ background: tag.color }} />
                <span>{tag.name}</span>
              </label>
            ))}
          </div>
