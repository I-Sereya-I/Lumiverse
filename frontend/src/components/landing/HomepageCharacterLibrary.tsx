...
export function HomepageCharacterLibrary() {
  ...
  return (
    ...
    >
      ...
      {loading && characters.length === 0 && <div className={styles.state}>Loading characters...</div>}
      {!loading && filter === 'this-chat' && !activeChatId && (
        <div className={styles.state}>No active chat is selected.</div>
      )}
      ...
      >
        ...
        >
          ...
          {!loading && activeChatId && filter === 'this-chat' && characters.length === 0 && (
            <div className={styles.state}>The active chat has no available characters.</div>
          )}
