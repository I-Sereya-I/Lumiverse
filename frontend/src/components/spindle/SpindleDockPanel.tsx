...
/**
 * CSS Modules run with `localsConvention: 'camelCaseOnly'`, so `.resize_left`
 * is only exported as `resizeLeft`. Looking the class up by its source name
 * silently yields `undefined` and leaves the handle unpositioned.
 */
const RESIZE_EDGE_CLASS: Record<SpindleDockEdge, string> = {
    ...
    >
      ...
      {!panel.collapsed && (
        <>
          ...
          {panel.resizable && (
            <div
              className={clsx(styles.resizeHandle, RESIZE_EDGE_CLASS[effectiveEdge])}
