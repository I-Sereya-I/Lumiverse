import { useEffect, useLayoutEffect, useMemo, useRef, useCallback, useState, type ReactNode, type RefObject } from 'react'
import { useTranslation } from 'react-i18next'
import { useParams } from 'react-router'
import { BookOpen, UserRound, ListChecks } from 'lucide-react'
...
import { chatLoreDockMode, chatTopDockMode, dockActionControlSize } from '@/lib/chatSurfaceLayout'
import { LorebookHalfScreenEditor } from '@/components/world-book-editor'
...
export default function ChatView() {
  ...
  const portraitDockVisible = portraitDockSettings.enabled && floatingAvatar !== null
  const portraitDockIsFloating = portraitDockSettings.dockSide === 'floating'
  const mobilePortraitDockVisible = isMobile && portraitDockVisible

  useEffect(() => {
    if (
      !settingsLoaded ||
      floatingAvatar ||
      !portraitDockSettings.enabled ||
      !portraitDockSettings.open ||
      !portraitDockSettings.pinned ||
      !portraitDockSettings.lastPortrait
    ) return
    openFloatingAvatar(
      portraitDockSettings.lastPortrait.imageUrl,
      portraitDockSettings.lastPortrait.displayName,
    )
  }, [floatingAvatar, openFloatingAvatar, portraitDockSettings, settingsLoaded])
  ...
  // Host for the two dock height variables. `.noticeDock` is a sibling of
  // `.chatColumnInner` (not a child, despite what the layout plan assumed), so
  // the vars have to be published one level up on `.chatColumn` to inherit into
  // both subtrees.
  ...
  return (
    ...
    >
      ...
      <div className={styles.body} {...(chatWidthMode !== 'full' ? { 'data-chat-constrained': '' } : {})}>
        {!isMobile && portraitDockVisible && !portraitDockIsFloating && portraitDockSettings.dockSide === 'left' && (
          <PortraitDock />
        )}
        ...
        {/*
          `data-lumiverse-surface` is the stable, un-hashed handle other surfaces
          query by. `LorebookHalfScreenEditor` measures the row it shares with this
          column and cannot use the CSS-Module class name; without this it would be
          left matching on `previousElementSibling`, which breaks silently if
          `.body`'s children are ever reordered.
        */}
        <div className={styles.chatColumn} ref={chatColumnRef} data-lumiverse-surface="chat-column">
          ...
          <div className={styles.chatColumnInner} style={innerStyle} data-select-mode={messageSelectMode || undefined}>
            <ChatTopDock hostRef={chatColumnRef}>
              <LorebookHalfEditorLauncher />
              <button
                type="button"
            ...
            <ChatLoreDock hostRef={chatColumnRef} />
        ...
        {!isMobile && portraitDockVisible && !portraitDockIsFloating && portraitDockSettings.dockSide === 'right' && (
          <PortraitDock />
        )}
      </div>
      ...
      {mobilePortraitDockVisible && (
        <>
          <button
            type="button"
            className={styles.portraitDockBackdrop}
            onClick={closeFloatingAvatar}
            aria-label="Close portrait dock"
          />
          <PortraitDock mobile />
        </>
      )}
      <ExpressionDisplay />
      {!isMobile && portraitDockVisible && portraitDockIsFloating && <PortraitDock />}
    </div>
  )
}

/**
 * Publishes `node`'s rendered height onto `host` as a CSS custom property while
 * `active`, and removes it again on cleanup. Measured with `offsetHeight` to
 * match how InputArea publishes `--lcs-input-safe-zone`.
 *
 * No feedback loop exists in either consumer: `--lcs-top-dock-height` only
 * offsets the absolutely positioned `.noticeDock`, and `--lcs-lore-dock-height`
 * only feeds `--lcs-chat-bottom-inset`, which moves MessageList's padding and
 * two absolutely positioned widgets. Neither can change the observed node's own
 * height (`.loreDock` is absolute; the strip is sized by its content).
 */
function useDockHeightVar(
  ...
  useLayoutEffect(() => {
    ...
    if (!active || !target || !observed) return

    const update = () => target.style.setProperty(name, `${observed.offsetHeight}px`)
  ...
  }, [host, node, name, active])
}

/**
 * Top dock. Hosts the quick toolbar plus whatever chrome ChatView passes as
 * children (the Lorebook launcher, the select-mode toggle).
 *
 * The V2 "settings-adjacent" variant renders card entries up to ~210px wide, so
 * it gets an in-flow strip across the top of the chat (`data-dock-mode="strip"`)
 * instead of the absolutely positioned top-right cluster. `actionCount` comes
 * from the same `useQuickToolbarActions()` hook the toolbar itself uses, because
 * QuickToolbar renders `null` for a zero-action toolbar and the strip must not
 * reserve an empty row for nothing.
 */
function ChatTopDock({ hostRef, children }: { hostRef: RefObject<HTMLElement | null>; children: ReactNode }) {
  const settings = useStore((state) => state.quickToolbarSettings)
  const { actions } = useQuickToolbarActions()
  const dockRef = useRef<HTMLDivElement | null>(null)

  const mode = chatTopDockMode({
    enabled: settings.enabled,
    variant: settings.variant,
    actionCount: actions.length,
  })

  useDockHeightVar(hostRef, dockRef, '--lcs-top-dock-height', mode === 'strip')
  ...
  return (
    <div
      ...
      data-lumiverse-surface="chat-top-dock"
      data-dock-mode={mode}
      style={strip
        ? ({
          '--lcs-dock-control-size': `${dockActionControlSize({
            iconSize: settings.v2IconSize,
            density: settings.v2Density,
          })}px`,
          // The <BookOpen>/<ListChecks> glyphs are hard-coded `size={14}`, which
          // reads as lost inside a card-height box. The stylesheet overrides the
          // rendered SVG box from this so they track the icon-size slider exactly
          // as the cards and the toolbar's gear do.
          '--lcs-dock-icon-size': `${settings.v2IconSize}px`,
        } as React.CSSProperties)
        : undefined}
    >
      {/* Own wrapper on purpose: it keeps the floating case byte-identical while
          letting strip mode size the toolbar independently of the buttons. */}
      <div className={styles.topDockToolbarSlot}>
        <QuickToolbar />
      </div>
      {/* Grouped so the two dock buttons keep their own tight rhythm and travel
          with the toolbar as one centred unit instead of being flung to the far
          right by a stretched slot. `display: contents` in floating mode leaves
          the pre-existing markup unchanged. */}
      <div className={styles.topDockActions}>{children}</div>
    </div>
  )
}

/**
 * Lore dock. Only the *wrapper* is conditional — `<LoreIndicator/>` is rendered
 * unconditionally in both branches. The default variant is `v2-compact`, which
 * renders its own `position: fixed` floating root and needs no dock; mounting
 * the indicator only inside the dock would make it vanish for the default
 * configuration.
 */
function ChatLoreDock({ hostRef }: { hostRef: RefObject<HTMLElement | null> }) {
  const settings = useStore((state) => state.loreIndicatorSettings)
  const dockRef = useRef<HTMLDivElement | null>(null)

  const mode = chatLoreDockMode({ enabled: settings.enabled, variant: settings.variant })

  useDockHeightVar(hostRef, dockRef, '--lcs-lore-dock-height', mode === 'strip')

  const indicator = <LoreIndicator />

  if (mode !== 'strip') return indicator

  return (
    <div ref={dockRef} className={styles.loreDock} data-lumiverse-surface="chat-lore-dock">
      {indicator}
    </div>
  )
}

function LorebookHalfEditorLauncher() {
  ...
  if (!settings.halfButtonEnabled) return null

  return (
    <button
      type="button"
      className={styles.toolbarBtn}
      onClick={() => openLorebookHalfEditor()}
      title="Open half-screen Lorebook Editor"
      aria-label="Open half-screen Lorebook Editor"
    >
      <BookOpen size={14} />
    </button>
  )
