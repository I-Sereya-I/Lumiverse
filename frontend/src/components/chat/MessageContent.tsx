...
function stripMarkdownImageDelimiters(raw: string): string {
  if (!/<[a-z][\w:-]*\b[^>]*>[\s\S]*!\[[^\]]*]\([^)]+\)/i.test(raw)) return raw

  const lines = raw.split('\n')
  const isDelimiter = (line: string) => /^-{3,}\s*$/.test(line)
  const isImportedImageLine = (line: string) =>
    /<[a-z][\w:-]*\b[^>]*>[\s\S]*!\[[^\]]*]\([^)]+\)/i.test(line)
  const nearestContentLine = (start: number, step: 1 | -1): string | null => {
  ...
  return lines
    .filter((line, idx) => {
      if (!isDelimiter(line)) return true
      const previous = nearestContentLine(idx - 1, -1)
      const next = nearestContentLine(idx + 1, 1)
      return !(
        (previous && isImportedImageLine(previous))
        || (next && isImportedImageLine(next))
      )
    })
    .join('\n')
}

function renderInlineMarkdownInHtmlText(raw: string): string {
  if (!/<[a-z][\w:-]*\b[^>]*>/i.test(raw) || !/[!*_`\[]/.test(raw)) return raw
  ...
  while (pos < raw.length) {
    ...
    const textEnd = nextTag < 0 ? raw.length : nextTag
    const text = raw.slice(pos, textEnd)
    parts.push(htmlDepth > 0 && skipDepth === 0 ? renderIslandInlineMarkdownText(text) : text)

    if (nextTag < 0) break

    const nextFence = fences[fenceIdx]
    if (nextFence && nextTag >= nextFence[0]) {
      parts.push(raw.slice(nextTag, nextFence[1]))
      pos = nextFence[1]
      fenceIdx++
      continue
    }

    const tagMatch = raw.slice(nextTag).match(/^<\/?([a-z][\w:-]*)\b[^>]*>/i)
    if (!tagMatch) {
      parts.push(raw[nextTag])
      pos = nextTag + 1
      continue
    }
    ...
    const isSelfClose = VOID_HTML_TAGS.has(tag) || /\/\s*>$/.test(token)

    parts.push(token)

    if (isClose) {
      htmlDepth = Math.max(0, htmlDepth - 1)
      if (/^(pre|code|script|style)$/i.test(tag)) skipDepth = Math.max(0, skipDepth - 1)
    } else if (!isSelfClose) {
      htmlDepth += 1
      if (/^(pre|code|script|style)$/i.test(tag)) skipDepth += 1
    }

    pos = nextTag + token.length
  }
...
function formatContent(raw: string): string {
  ...
  const delimiterSafe = stripMarkdownImageDelimiters(normalized)
  const htmlMarkdownSafe = renderInlineMarkdownInHtmlText(delimiterSafe)
  const listSafe = escapeIsolatedOrderedListItems(htmlMarkdownSafe)
