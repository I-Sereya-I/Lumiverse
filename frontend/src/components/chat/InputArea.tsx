...
export default function InputArea({ chatId }: InputAreaProps) {
  ...
  const [connectionsPickerOpen, setConnectionsPickerOpen] = useState(false)
  ...
  const connectionsPickerLauncherRef = useRef<HTMLButtonElement>(null)
  ...
  const connectionsPickerSettings = useStore((s) => s.connectionsPickerSettings)
  ...
  const handleSTTToggle = useCallback(async () => {
    ...
    if (voiceSettings.sttProvider === 'connection' && !voiceSettings.sttConnectionId) {
      ...
      // This used to route through `openModal` with a `settings` name, which was
      // a dead end: `ModalContainer` has no such branch, so it set `activeModal`
      // with nothing on screen and nothing able to clear it again. The settings
      // modal is its own store field (`settingsModalOpen`), opened this way.
      // The STT connection picker lives in the `voice` tab.
      useStore.getState().openSettings('voice')
      return
    }
  ...
  return (
    ...
    >
      ...
      <div data-spindle-mount="chat_toolbar">
        ...
        {!isStreaming && (
          <div className={styles.actionBar}>
            ...
            {connectionsPickerSettings.enabled && connectionsPickerSettings.launcherEnabled && (
              <button
                type="button"
                className={clsx(styles.actionBtn, connectionsPickerOpen && styles.actionBtnActive)}
                onClick={() => setConnectionsPickerOpen((value) => !value)}
                title="Connections Picker"
                aria-label="Open Connections Picker"
                aria-expanded={connectionsPickerOpen}
                ref={connectionsPickerLauncherRef}
              >
                <Waypoints size={14} />
              </button>
            )}
      ...
      <ConnectionsPicker
        open={connectionsPickerOpen}
        onClose={() => setConnectionsPickerOpen(false)}
        anchorElement={connectionsPickerLauncherRef.current}
      />
              ...
              {effectiveAttachedGlobalAddons.length > 0 && (
                <>
                  ...
                  {effectiveAttachedGlobalAddons.map((addon) => (
                    ...
                    >
                      <span className={styles.personaMain}>
                        <Globe size={13} style={{ opacity: addon.enabled ? 1 : 0.4, color: addon.enabled ? 'var(--lumiverse-info)' : undefined }} />
                      ...
                      {avatarUrl ? (
                        <img
                          ...
                          style={{
                            ...
                            objectFit: 'cover',
                            flexShrink: 0,
                            background: 'var(--lumiverse-surface-muted)',
                          }}
                        />
                      ) : (
                        <span
