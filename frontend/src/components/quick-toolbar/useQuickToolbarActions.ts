...
import { Puzzle, Settings, Zap } from 'lucide-react'
...
export const DESIGN_DEFAULT_IDS = ['profile', 'connections', 'council', 'lorebook', 'presets', 'settings']
...
export function useQuickToolbarActions() {
  ...
  const actionCatalog = useMemo(() => {
    ...
    const registeredActions: ToolbarAction[] = [
      ...
      run: () => runSurface({ kind: 'command' }, () => void command.run(navigate)),
    }))
    const extensionInputActions: ToolbarAction[] = inputBarActions.map((action) => ({
      id: `input-action:${action.extensionId}:${action.id}`,
      label: action.label,
      ...
      icon: action.iconSvg || action.iconUrl ? Puzzle : Zap,
      surface: { kind: 'command' },
      run: () => runSurface(
        { kind: 'command' },
  ...
  const visibleIds = useMemo(() => {
    if (settings.visibleTabIds.length === 0 || arraysEqual(settings.visibleTabIds, LEGACY_DEFAULT_IDS)) {
  ...
  const orderedIds = useMemo(() => {
    const configuredOrder = settings.iconOrder.length === 0 || arraysEqual(settings.iconOrder, LEGACY_DEFAULT_IDS)
      ? DESIGN_DEFAULT_IDS
      : settings.iconOrder
    return [
