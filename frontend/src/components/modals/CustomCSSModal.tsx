...
import { resolveComponentReference } from '@/lib/componentRegistryJoin'
...
export default function CustomCSSModal() {
  ...
  // ── Reference-pane join (pure, see componentRegistryJoin.ts) ──
  // `CSS_MODULE_REGISTRY` (keyed by stylesheet basename) and the generated maps
  // are two separate enumerations; this is the single point where they meet.
  const reference = useMemo(
    () => resolveComponentReference(selected, GENERATED_COMPONENT_CSS, componentTemplate?.props),
        ...
        <div className={styles.body}>
          ...
          <div className={styles.main}>
            ...
            <div className={styles.editorContentRow}>
              ...
              {/* Never gated on the generated data being present: a component the
                  picker lists always gets a pane, even if one half is missing. */}
              {showReference && !isGlobal && (
                <div className={styles.editorSidebar}>
                  {activeTab === 'css'
                    ? <ComponentCssReference
                        componentName={selected}
                        cssContent={reference.cssContent}
                        hasStylesheet={reference.hasStylesheet}
                      />
                    : <PropsReference props={reference.props} componentName={selected} />}
                </div>
              )}
            </div>
