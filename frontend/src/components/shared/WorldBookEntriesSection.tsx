...
function SortableEntryRow({
  ...
  return (
    // `data-entry-id` sits on the wrapper, not the inner `.entryRow`, so the
    // reveal query in WorldBookEntriesSection targets a box that includes the
    // inline editor mounted below the row.
    <div
      ref={setNodeRef}
      style={style}
      data-entry-id={entry.id}
      className={clsx(isDragging && styles.rowDragging)}
    >
      <div
        className={clsx(
          styles.entryRow,
...
export default function WorldBookEntriesSection({
  ...
  // Scopes the reveal query to this list. The lorebook editor's table renders
  // its own `data-entry-id` rows and can be mounted at the same time, so a bare
  // `document.querySelector` would be able to match the wrong surface.
  // `.entryList` is not itself a scroll container — `scrollIntoView` walks up
  // and finds the real one, which this component does not own.
    ...
    try {
      ...
      if (pendingEntryId && !nextEntries.some((entry) => entry.id === pendingEntryId)) {
        try {
          ...
          useStore.getState().setPendingWorldBookEditEntryId(null)
        }
      }
  ...
  // Applying the persisted view prefs depends on the three PRIMITIVE fields for
  // this book, never on the `worldBookEntryViewPrefs` map itself. The store
  // hands back a fresh object identity on every settings write (and again when
  // `loadSettings` hydrates), so an object dependency re-ran this effect for
  // unrelated books and unrelated settings.
  const bookViewPref = worldBookEntryViewPrefs[selectedBookId]
  const prefSortBy = bookViewPref?.sortBy ?? 'custom'
  const prefSortDir = bookViewPref?.sortDir ?? 'asc'
  const prefPageSize = bookViewPref?.pageSize || DEFAULT_PAGE_SIZE

  useEffect(() => {
    setEntrySortBy(prefSortBy)
    setEntrySortDir(prefSortDir)
    setEntryPageSize(prefPageSize)
  }, [prefSortBy, prefSortDir, prefPageSize])

  // Resetting the transient view state is keyed on the BOOK alone. Sort order,
  // sort direction and page size deliberately do not clear `selectedEntryId`:
  // they reorder the same entry set, so collapsing the open editor is pure
  // loss. Bundling that reset in with the prefs read is what silently closed
  // the entry the user was editing whenever they touched the sort or page-size
  // control. An entry that falls off the visible page is dropped by the
  // presence prune below instead, and paging clears the selection at its own
  // call site.
  useEffect(() => {
    setEntryPage(1)
  ...
  useEffect(() => {
    ...
    // Never strand the open editor on a page, filter or sort window that no
    // longer lists it. This runs only when `entries` itself changes, so a
    // selection set optimistically before a reload (deep link, new entry)
    // survives until the fetched list arrives and can be checked against it.
    setSelectedEntryId((current) => (
      current && !entries.some((entry) => entry.id === current) ? null : current
    ))
  }, [entries])

  // Reveal the entry that was just opened. The inline editor mounts in the same
  // commit, so wait one frame for it to gain height before scrolling — the same
  // shape as hooks/useEditKeyboard.ts. Native `scrollIntoView` computes its
  // delta inside the element's own layout space and is therefore immune to the
  // `zoom: var(--lumiverse-ui-scale)` layer in theme/reset.css: no
  // `readUiScale()` conversion belongs here, and adding one would be the bug.
  useEffect(() => {
  ...
  const updateEntry = useCallback((entryId: string, updates: Record<string, any>) => {
    ...
    void worldBooksApi.updateEntry(selectedBookId, entryId, {
      ...updates,
      expected_revision: current.revision,
    })
      .then(async (updated) => {
        setEntries((items) => items.map((entry) => entry.id === entryId ? updated : entry))
        await refreshVectorSummary()
      })
      .catch(() => {
        void refetchCurrentPage()
      })
  }, [entries, selectedBookId, refreshVectorSummary, refetchCurrentPage])
  ...
  const handleDeleteEntries = useCallback(async (entryIds: string[]) => {
    if (entryIds.length === 1) {
      const current = entries.find((entry) => entry.id === entryIds[0])
      if (!current) return
      await worldBooksApi.deleteEntry(selectedBookId, entryIds[0], current.revision)
    } else {
      await worldBooksApi.bulkEntryAction(selectedBookId, {
        action: 'delete',
        entry_ids: entryIds,
        expected_revisions: expectedRevisionMap(entries, entryIds),
      })
    }
    ...
    await refreshVectorSummary()
  }, [entries, selectedBookId, refetchCurrentPage, refreshVectorSummary])
    ...
    try {
      if (moveCopyState.mode === 'move') {
        await worldBooksApi.bulkEntryAction(selectedBookId, {
          ...
          expected_revisions: expectedRevisionMap(entries, moveCopyState.entryIds),
        })
  ...
  }, [entries, moveCopyState, moveTargetBookId, selectedBookId, refetchCurrentPage, refreshVectorSummary])
  ...
  const handleBulkRenumber = useCallback(async () => {
    ...
    try {
      const payload: WorldBookEntryBulkActionInput = {
        ...
        expected_revisions: expectedRevisionMap(entries, renumberState.entryIds),
      }
  ...
  }, [entries, renumberDirection, renumberStart, renumberState, renumberStep, selectedBookId, refetchCurrentPage])
  ...
  const handleBulkKeyword = useCallback(async () => {
    ...
    try {
      await worldBooksApi.bulkEntryAction(selectedBookId, {
        ...
        expected_revisions: expectedRevisionMap(entries, keywordState.entryIds),
      })
  ...
  }, [entries, keywordState, keywordValue, keywordTarget, selectedBookId, refetchCurrentPage, refreshVectorSummary])
  ...
  const handleBulkSetPosition = useCallback(async () => {
    ...
    try {
      const payload: WorldBookEntryBulkActionInput = {
        ...
        expected_revisions: expectedRevisionMap(entries, positionState.entryIds),
  ...
  }, [entries, positionState, bulkPosition, bulkDepth, selectedBookId, refetchCurrentPage])
  ...
  const handleDragEnd = useCallback(async ({ active, over }: any) => {
    ...
    try {
      await worldBooksApi.reorderEntries(selectedBookId, {
        ordered_ids: nextEntries.map((entry) => entry.id),
        expected_revisions: expectedRevisionMap(entries, entries.map((entry) => entry.id)),
      })
      await refetchCurrentPage()
    } catch {
      await refetchCurrentPage()
  ...
  return (
    <>
      ...
      {moveCopyState && (
        <ModalShell data-component="WorldBookEntriesSection" isOpen={true} onClose={() => !pendingAction && setMoveCopyState(null)} maxWidth="520px">
      ...
      {renumberState && (
        <ModalShell data-component="WorldBookEntriesSection" isOpen={true} onClose={() => !pendingAction && setRenumberState(null)} maxWidth="520px">
          <div className={styles.dialogBody}>
      ...
      {keywordState && (
        <ModalShell data-component="WorldBookEntriesSection" isOpen={true} onClose={() => !pendingAction && setKeywordState(null)} maxWidth="520px">
          <div className={styles.dialogBody}>
      ...
      {positionState && (
        <ModalShell data-component="WorldBookEntriesSection" isOpen={true} onClose={() => !pendingAction && setPositionState(null)} maxWidth="520px">
          <div className={styles.dialogBody}>
