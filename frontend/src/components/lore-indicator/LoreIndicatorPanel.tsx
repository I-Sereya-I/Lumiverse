...
const ACTIVATION_TYPES = ['constant', 'keyword', 'vector'] as const
  ...
  const groups = useMemo(() => {
    ...
    for (const entry of visibleEntries) {
      const label = mode === 'expanded' && groupBy === 'type'
        ? TYPE_LABELS[entry.activationType]
        ...
        {compact ? (
          <div className={styles.compactEntries}>
            {(settings.v2BookDisplay === 'grouped'
              ...
              const appearance = settings.entryTypeAppearance[entry.activationType]
              const TypeIcon = CONFIGURED_ICONS[appearance.icon as keyof typeof CONFIGURED_ICONS] ?? TYPE_ICONS[entry.activationType]
  ...
  return (
    <aside className={styles.detail}>
      ...
      <dl>
        ...
        <dd>{entry.estimatedTokens.toLocaleString()} tokens</dd>
      </dl>
    </aside>
  )
}
