...
export function ConnectionsPicker({ open, onClose, anchorElement }: ConnectionsPickerProps) {
  ...
  const settings = { ...DEFAULT_CONNECTIONS_PICKER_SETTINGS, ...storedSettings }
  ...
  const rememberVariantRect = (variant: ConnectionsPickerVariant, rect: SurfaceRectPrefs) => {
  ...
  // Normalize only when profiles or the stored tag catalog actually changes.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profiles, settings.profileTags])
  ...
  // Center each variant only once. Subsequent opens restore that variant's rectangle.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [anchorElement, open, settings.positionInitialized, settings.variant])
