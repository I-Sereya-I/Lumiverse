...
export function generateThemeVariables(
  ...
  vars['--lumiverse-primary-005'] = hsla(h, s, pL, 0.05)
  vars['--lumiverse-primary-008'] = hsla(h, s, pL, 0.08)
  ...
  vars['--lumiverse-primary-025'] = hsla(h, s, pL, 0.25)
  vars['--lumiverse-primary-030'] = hsla(h, s, pL, 0.3)
  ...
  vars['--lumiverse-warning-030'] = rgba(245, 158, 11, 0.3)
  vars['--lumiverse-warning-050'] = rgba(245, 158, 11, 0.5)
  vars['--lumiverse-error'] = vars['--lumiverse-danger']
  vars['--lumiverse-error-010'] = vars['--lumiverse-danger-010']
  // Fourth status hue. Unlike its three siblings this one is clamped for light
  // mode: the historical literal (#42a5f5) is a 61%-lightness blue that only
  // reaches ~2.7:1 against the L96 light-mode surface, and most of its 32 call
  // sites use it as `color:`. ensureReadable caps it at 35% lightness there and
  // leaves the dark-mode value byte-identical to what those call sites rendered
  // before the token existed.
  vars['--lumiverse-info'] = isDark ? info : ensureReadable(info, false)
  ...
  if (bc) {
    if (bc.primary) {
      ...
      vars['--lumiverse-primary-005'] = hexRgba(primary, 0.05)
      vars['--lumiverse-primary-008'] = hexRgba(primary, 0.08)
      ...
      vars['--lumiverse-primary-025'] = hexRgba(primary, 0.25)
      vars['--lumiverse-primary-030'] = hexRgba(primary, 0.3)
    ...
    if (bc.warning) {
      ...
      vars['--lumiverse-warning-030'] = hexRgba(warning, 0.3)
  ...
  // ── Derived-last tokens ──
  // Bare `R, G, B` triple of whatever --lumiverse-primary finally resolved to, for
  // the call sites that compose their own alpha via `rgba(var(--…-rgb), 0.05)`.
  // MUST be emitted after the baseColors block: computing it from the hsla() above
  // would freeze it at the preset accent and ignore a `baseColors.primary`
  // override, which is the exact phantom-token failure this replaces.
  const primaryRgb = parseColorToRgb(vars['--lumiverse-primary'])
  vars['--lumiverse-primary-rgb'] = primaryRgb ? primaryRgb.join(', ') : '147, 112, 219'
  // Foreground for a --lumiverse-primary fill, plus the three aliases that were
  // each independently frozen at `#fff`. Same reason for the placement.
  vars['--lumiverse-primary-fg'] = vars['--lumiverse-primary-contrast']
  vars['--lumiverse-on-primary'] = vars['--lumiverse-primary-contrast']
  vars['--lumiverse-accent-fg'] = vars['--lumiverse-primary-contrast']
