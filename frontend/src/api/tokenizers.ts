...
export const tokenizersApi = {
  ...
  /**
   * `token_count` is `null` when no tokenizer pattern matches the model — that is
   * a normal answer, not an error, and the caller must fall back to the heuristic
   * *without* persisting the result.
   *
   * `options` exists so background prefetch work can be cancelled: the lorebook
   * editor's token scheduler passes an `AbortSignal` from a per-book controller
   * and aborts on book change, editor close and profile change.
   */
