 ...
 *
 * Everything is resolved against `available` — the tabs actually rendered — not
 * the full set, or an arrow press in a shrunken tablist would select a tab that
 * has no button to move focus to. With a single available tab every owned key
 * resolves to that tab (focus stays put, the key is still consumed).
 */
