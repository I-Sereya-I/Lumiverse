...
/**
 * `v3-adaptive` was removed: it differed from `v1-free` by four cosmetic CSS
 * rules and owned no settings of its own, so it was a second name for the same
 * toolbar. Rows persisted with the old value are rewritten to `v1-free` by the
 * migration in `store/slices/settings.ts`'s `loadSettings`.
 */
export type QuickToolbarVariant = 'v1-free' | 'v2-settings-adjacent'
/**
 * V2 card density. `comfortable` is the confirmed two-line card design (icon +
 * heading + value); `compact` is a single-line icon + short label row with
 * tighter padding. Only V2 reads it — the free toolbar has `scale`,
 * `orientation` and `iconSize` instead.
 */
...
export type LorebookTokenCountMode = 'live' | 'delayed' | 'manual'
...
/** Extents without a position — see `QuickToolbarSettings.verticalSize`. */
...
export interface QuickToolbarSettings {
  ...
  opacity: number
  snapToEdge: boolean
  /** Shows the eight drag-to-resize handles on the free-floating toolbar. */
  resizeHandlesEnabled: boolean
  /**
   * The shared position **and the horizontal orientation's extents**. A
   * non-positive `width`/`height` is the **auto sentinel**: "size me from my
   * content" (see `lib/quickToolbarGeometry.ts`'s `isAutoRect`). `x`/`y` are
   * always meaningful, and are the *only* stored position — see `verticalSize`.
   */
  rect: SurfaceRectPrefs
  /**
   * The vertical orientation's own extents, so each orientation remembers the
   * size it was given. Deliberately position-less: `x`/`y` live on `rect` alone
   * and are shared, because flipping orientation must never teleport the
   * toolbar.
   *
   * Without this the two orientations shared one rect, and `clampSurfaceRect`
   * raised the new orientation's main axis on a flip without ever lowering the
   * stale cross axis — so a pinned 560x46 horizontal toolbar rendered 560x500
   * when flipped to vertical. Read through `selectToolbarRect` and written
   * through `withToolbarRect` (`lib/quickToolbarGeometry.ts`).
   *
   * Needs **no migration of its own**: `mergeStoredSetting` recurses into plain
   * objects and backfills missing keys from the default *before* migrations
   * run, and the default — the auto sentinel — is exactly the value an existing
   * row wants.
   */
  verticalSize: SurfaceSizePrefs
  /**
   * Schema version of the stored geometry. Version 1 rows hold the pre-auto-
   * sizing `368x54` box, which never had any visual effect; version 2 rewrote
   * them to the auto sentinel. Version 3 re-autos **both** orientations,
   * because a row flattened by a resize drag under the shared-rect bug is
   * byte-indistinguishable from a size the user deliberately chose.
   *
   * The default is deliberately **1**, not the current version:
   * `mergeStoredSetting` backfills keys a stored row is missing from the
   * default, so a default at the current version would make every legacy row
   * read as already-migrated and the migration could never fire.
   */
  rectVersion: number
  /**
   * Leaves a small edge tab on screen while a full-screen modal hides the
   * free-floating toolbar, so it can be brought back without closing the modal.
   *
   * Off by default and deliberately so: the tab is portaled at the toolbar's
   * own z-index, which puts it over small `ModalShell` dialogs too — a
   * "Delete this entry?" confirmation would get a toolbar tab pasted on it.
   * Opt-in is the accepted mitigation for that trade-off.
   *
   * Unlike `rectVersion` this needs **no migration**: `mergeStoredSetting`
   * backfills keys a stored row is missing from this default, so every existing
   * row reads `false` on first load, which is also the intended value.
   */
  modalRestoreHandle: boolean
  v2IconSize: number
  v2LabelTextSize: number
  /**
   * V2 renders context cards whose heading is the label, so it defaults to
   * visible — unlike the icon-only V1 toolbar that uses `labelVisible`.
   */
  v2LabelVisible: boolean
  /**
   * V2-only. Same no-migration reasoning as `modalRestoreHandle`: a stored row
   * without the key merges to `'comfortable'`, which is the shipped look, so
   * nobody's toolbar changes on upgrade.
   */
...
export interface LoreIndicatorSettings {
  ...
  iconSize: number
  textSize: number
  entryTypeAppearance: Record<'constant' | 'keyword' | 'vector', { color: string; icon: string }>
  v4Items: Array<{ id: string; visible: boolean; removed: boolean; mode: 'icon' | 'iconText'; order: number }>
  v4Spacing: number
  /** How the V4 expanded panel groups activated entries. */
  v4GroupBy: LoreIndicatorGroupBy
  /** Entries listed per group in the V4 panel before the "+N more" affordance. */
  v4BookPreviewCount: number
  /** Show the keyboard-shortcut footer in the V5 command palette. */
...
export interface LorebookEditorSettings {
  ...
  /**
   * How the half-screen editor is laid out.
   *
   * `docked` (default) is the original behaviour: a right-docked flex child of the
   * chat row at `height: 100%`, where only `halfRect.width` is meaningful.
   * `floating` renders it inside `ResizablePanelFrame` using the whole `halfRect`,
   * so it drags and resizes from eight handles like the full editor. Both are
   * offered because they suit different screen shapes; the mode is a segmented
   * control in the Lorebook Editor settings card.
   */
  halfEditorMode: 'docked' | 'floating'
  fullRect: SurfaceRectPrefs
  /**
   * In `docked` mode only `width` is read — the host is `height: 100%` in the chat
   * flex row and has no x/y of its own. All four fields are read in `floating` mode,
   * which is why the height/X/Y sliders are gated on the mode rather than deleted.
   */
  halfRect: SurfaceRectPrefs
  /**
   * Chat width the half editor must always leave behind, in layout px.
   *
   * The hard-coded predecessor was 240, which let a 1920px row hand the editor
   * 1680px and leave chat a sliver — the "it covers everything" report. Raising the
   * default to 420 is the actual fix; `resolveHalfEditorLayout` takes it as a
   * parameter so the user can tune it.
   */
  minChatWidth: number
  /**
   * Smallest width either editor may be squeezed to before it stops adapting and
   * starts overlaying. Also raises the full editor's own minimum when set above it.
   */
  ...
  /**
   * When the entry editor estimates tokens. `live` recounts while typing,
   * `delayed` waits `tokenCountDelayMs` after an entry is opened, and
   * `manual` only counts when the Count tokens button is pressed.
   */
  tokenCountMode: LorebookTokenCountMode
  tokenCountDelayMs: number
  /**
   * Prefetch exact counts for rows the user is about to open: a hover that
   * survives {@link tokenPrefetchHoverDelayMs}, plus the rows either side of the
   * selection (arrow-key navigation has no hover signal).
   *
   * Orthogonal to {@link tokenCountMode}, which decides when the **open** entry is
   * counted *and saved*. Prefetched counts are client-only and are **never**
   * written back, so they cannot bump `revision`.
   */
  tokenPrefetchHover: boolean
  /** Hover dwell before a prefetch is issued, in ms. Range 0–1000. */
  tokenPrefetchHoverDelayMs: number
  /**
   * Count every entry of the **currently open book** in the background, so the
   * list shows exact numbers instead of `~` estimates.
   *
   * Scoped to `selectedBookId` and aborted the moment the selection changes — it
   * never walks the library. Like the hover prefetch, nothing it computes is
   * saved, so `manual` + this is a legitimate combination: nothing is ever
   * auto-written, but the list is exact.
   */
  tokenCountAllEntries: boolean
  /**
   * Revision of `visibleEntryMetadata` this install has been migrated to.
   * Stored arrays replace defaults wholesale, so newly added column ids are
   * backfilled once and the version recorded — see `lorebookEntryColumns`.
   * Defaults to 0 so installs predating the field run the migration.
   */
...
export interface SettingsSlice {
  ...
  landingPageChatsDisplayed: number
  landingPageLayoutMode: 'cards' | 'compact'
  /** Which landing-page tab is showing. New key — backfilled by mergeStoredSetting. */
  landingPageActiveTab: 'characters' | 'chats'
  charactersPerPage: number
...
export interface WorldInfoSlice {
  ...
  /** Entry id the Lorebook tab should expand after selecting its owning book. */
