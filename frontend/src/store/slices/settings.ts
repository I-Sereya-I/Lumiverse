...
import { DEFAULT_LANDING_PAGE_TAB } from '@/lib/landingPageTabs'
import { repairToolbarRects } from '@/lib/quickToolbarGeometry'
...
export const createSettingsSlice: StateCreator<AppStore, [], [], SettingsSlice> = (set, get) => ({
  ...
  landingPageActiveTab: DEFAULT_LANDING_PAGE_TAB,
  ...
  loadSettings: async () => {
    try {
      ...
      let migratedQuickToolbar = false
      ...
      // Quick toolbar migrations. Both are no-ops when the user has never saved
      // toolbar settings — `patch.quickToolbarSettings` is then absent and the
      // in-memory default (already `rectVersion: 1` with an auto rect) stands
      // untouched, so a first-time visitor triggers no spurious write.
      //
      //  1. `v3-adaptive` no longer exists. `mergeStoredSetting` replaces
      //     primitives wholesale, so the dead value survives the merge and has
      //     to be rewritten here.
      //  2. The stored geometry is re-auto'd whenever `rectVersion` is behind
      //     `TOOLBAR_RECT_VERSION` (currently 3). `mergeStoredSetting` merges
      //     `rect` key-by-key, so a new default alone would never reach an
      //     existing row — this migration is what actually resets it to the
      //     auto sentinel, preserving the `x`/`y` the user may have dragged.
      //
      //     Version 3 is the per-orientation-size repair. Under the old shared
      //     rect a resize drag wrote the *union* of both orientations' boxes,
      //     and the result is byte-indistinguishable from a size the user chose
      //     on purpose — so both orientations are reset. Accepted cost: someone
      //     who deliberately pinned a size loses it once, on one load, and gets
      //     auto-fit back. `repairToolbarRects` returns its input by identity
      //     when the row is already current, so the `next !== qt` check below
      //     keeps this a one-shot.
      if (patch.quickToolbarSettings) {
        const qt = patch.quickToolbarSettings
        let next = qt
        if ((next.variant as string) === 'v3-adaptive') {
          next = { ...next, variant: 'v1-free' }
        }
        next = repairToolbarRects(next, DEFAULT_QUICK_TOOLBAR_SETTINGS)
        if (next !== qt) {
          patch.quickToolbarSettings = next
          migratedQuickToolbar = true
          // Mirror the correction into the pending row so the re-`putMany`
          // below cannot write the pre-migration value straight back.
          if (pendingKeys && 'quickToolbarSettings' in pendingKeys) {
            pendingKeys.quickToolbarSettings = next
          }
        }
      }
      ...
      if (migratedQuickToolbar) {
        settingsApi.put('quickToolbarSettings', patch.quickToolbarSettings).catch(() => {})
      }
